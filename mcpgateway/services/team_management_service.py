...
class TeamNameConflictError(TeamManagementError):
    """Active-slug collision. Kept distinct from TeamManagementError so the router can decide,
    based on caller authorization, how much detail to disclose — unlike other
    ...
    async def create_team_with_members(
        ...
        try:
            ...
            if existing_inactive_team:
                ...
                # Reaching here means there is no inactive team with this slug. Defensively check
                # for an ACTIVE team with the same slug before inserting: EmailTeam.slug is globally
